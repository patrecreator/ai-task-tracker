...
export async function parseTaskFromText(rawInput: string, now: Date): Promise<ParsedTask> {
  ...
  if (!trimmed) {
    return { title: "Без назви", priority: "medium", deadline: null };
  }
  ...
  try {
    ...
    return { title: trimmed.slice(0, 200), priority: "medium", deadline: null };
  }
...
JSON shape exactly:
{"title":"string","priority":"high"|"medium"|"low","deadline":"2026-05-02T12:00:00.000Z"|null}
...
export async function suggestTopTasks(
  ...
  const lines = open.map(
    (t) =>
      `id=${t.id} | title=${JSON.stringify(t.title)} | priority=${t.priority} | deadline=${t.deadline?.toISOString() ?? "none"} | raw=${JSON.stringify(t.rawInput)}`,
  );
...
export async function dailyDigest(
  ...
  const body = open
    .map(
      (t) =>
        `- ${JSON.stringify(t.title)} [${t.priority}] deadline=${t.deadline?.toISOString().slice(0, 10) ?? "none"}`,
    )
    .join("\n");
